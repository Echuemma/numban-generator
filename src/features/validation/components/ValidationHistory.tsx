...
export const ValidationHistory: React.FC = () => {
  ...
  const handleRemoveValidation = (id: string) => {
    const validation = filteredHistory.find((v) => v.id === id);
  ...
  const confirmDelete = () => {
    if (selectedValidation?.id) {
      dispatch(removeValidation(selectedValidation.id));
    }
    setModalOpen(false);
  ...
  return (
    <>
      ...
      <ConfirmModal
        ...
        message={`Are you sure you want to delete validation for NUBAN ${selectedValidation?.nuban}? This action cannot be undone.`}
        confirmText="Delete"
        cancelText="Cancel"
